<script lang="ts">
  ...
  let { title, value, hint = '', trend = '', accent = 'emerald' } = $props<{
    title: string;
    value: string | number;
    hint?: string;
    trend?: string;
  ...
  const accentClasses: Record<'emerald' | 'blue' | 'purple' | 'red', string> = {
...
<Card size="xl" class={`h-full rounded-2xl border bg-gradient-to-br ${accentClasses[accent as keyof typeof accentClasses]} bg-slate-900/70 p-4 shadow-lg shadow-slate-950/20`}>
  <div class="flex items-start justify-between gap-3">
    ...
    {#if trend}
      <Badge color="gray" class="rounded-full border border-slate-700 bg-slate-950/60 text-slate-200">{trend}</Badge>
    {/if}
  </div>
</Card>
