...
- Temporal keeps one normal productive workflow per Paperless document ID. Each new document workflow owns optional tag-gated OCR, target embedding, classification, judge, review wait and the accepted Paperless update. Explicit force reprocessing supersedes the waiting review workflow and uses a separate generation. The retired singleton model-phase path remains registered only for replay of existing histories.
