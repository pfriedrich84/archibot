...
- Temporal is the target sole owner of productive workflow execution, timers, retry, heartbeat and recovery under ADR-0022. Laravel owns authorization, HTTP/UI and transactional workflow intents; Python relays those intents and owns Temporal workflows/activities and document processing. ArchiBot PostgreSQL stores business data and UI projections, while Temporal uses dedicated persistence for execution history.
- Processing waits for a complete embedding index. Only documents without the inbox tag are trusted classification context.
- Temporal keeps one stable productive Workflow ID per Paperless document ID. Each document workflow owns optional tag-gated OCR, target embedding, classification, judge, review wait and the accepted Paperless update. Explicit force reprocessing supersedes the current pipeline generation and uses Continue-As-New for a new Temporal Run under the same Workflow ID. No global model-phase scheduler participates in productive execution.
