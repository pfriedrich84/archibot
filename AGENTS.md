...
- Temporal keeps one normal productive workflow per Paperless document ID. Each document workflow owns optional tag-gated OCR, target embedding, classification, judge, review wait and the accepted Paperless update. Explicit force reprocessing supersedes the waiting review workflow and uses a separate generation. No global model-phase scheduler participates in productive execution.
