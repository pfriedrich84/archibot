...
- Temporal keeps one normal productive workflow per Paperless document ID. A singleton model-phase scheduler drains all currently eligible embedding work before classification work, and all classification work before judge work. New documents that miss a phase boundary wait for the next cycle; inactive model phases must not dispatch provider work. Explicit force reprocessing uses a separate generation.
