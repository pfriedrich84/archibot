...
def test_absurd_backend_supports_queue_name_signatures(monkeypatch):
    ...
    monkeypatch.setattr(absurd_queue, "Absurd", FakeClient)

    backend = absurd_queue._AbsurdBackend(
        queue_name="archibot",
    ...
    assert registered[1] == ("create_queue", "archibot.io")
    assert registered[2][0:3] == ("register_task", "<lambda>", "archibot.io")
    assert spawned == [
        (
            "<lambda>",
            {
...
def test_absurd_backend_starts_registered_queue_workers(monkeypatch):
    started = []
    ...
    class FakeClient:
        ...
        def register_task(self, name, queue):
            def decorator(func):
                return func

            return decorator

        def spawn(self, task_name, payload, queue):
            pass

        def start_worker(self, concurrency, claim_timeout):
            started.append((self.queue_name, concurrency, claim_timeout))
    ...
    backend.actor(queue_name="archibot.webhook")(lambda value: value)
    backend.actor(queue_name="archibot.io")(lambda value: value)

    backend.start_worker(concurrency=2, claim_timeout=30)

    assert sorted(started) == [
        ("archibot", 2, 30),
        ("archibot.io", 2, 30),
        ("archibot.webhook", 2, 30),
    ]
