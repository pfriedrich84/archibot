...
<div class="space-y-6">
    ...
    <div class="grid gap-4 md:grid-cols-3">
        <div class="rounded-xl border p-4">
            ...
            <div class="mt-2 text-3xl font-semibold">{snapshot.total_embedded}</div>
        </div>
        ...
        {#if latestReindexJob}
            <div class="space-y-3 p-4 text-sm">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="rounded-full bg-muted px-2 py-0.5">{latestReindexJob.status}</span>
                    <span class="text-muted-foreground">
                        Phase: {latestReindexJob.progress.phase ?? '—'} · {latestReindexJob.progress.done ?? 0}/{latestReindexJob.progress.total ?? 0}
                    </span>
                    {#if latestReindexJob.progress.failed}
                        <span class="text-destructive">{latestReindexJob.progress.failed} failed</span>
                    {/if}
                </div>
            ...
            <div class="grid gap-2 border-b p-4 text-sm last:border-b-0">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    {#if item.created_date}
                        <span class="text-muted-foreground">{item.created_date}</span>
                    {/if}
                </div>
                <div>{item.title || 'Untitled'}</div>
                <div class="flex flex-wrap gap-2 text-xs text-muted-foreground">
                    {#if item.correspondent}<span>Correspondent #{item.correspondent}</span>{/if}
                    {#if item.doctype}<span>Type #{item.doctype}</span>{/if}
                    {#if item.storage_path}<span>Storage #{item.storage_path}</span>{/if}
                    {#if item.tags.length > 0}<span>Tags: {item.tags.join(', ')}</span>{/if}
                    {#if item.indexed_at}<span>Indexed {item.indexed_at}</span>{/if}
                </div>
            </div>
        {:else}
            <div class="p-8 text-center text-muted-foreground">
