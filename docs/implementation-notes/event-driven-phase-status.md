...
- Existing Laravel webhook routes `/webhook/new` and `/webhook/edit` use the legacy worker path, may be removed, and must not be extended for the new architecture. The target endpoint is `/api/webhooks/paperless` with `/webhook` as a simple alias.
