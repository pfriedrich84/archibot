...
- Phase 4 Dramatiq foundation: partially implemented.
  ...
  - Laravel webhook ingestion can now optionally attempt direct enqueue for newly persisted non-duplicate deliveries through `ARCHIBOT_WEBHOOK_ENQUEUE_COMMAND`. A matching Python one-shot command exists: `python -m app.event_worker enqueue-webhook --delivery-id={delivery_id}`. Failures keep the delivery `queued`, emit redacted `webhook.enqueue_deferred`, and rely on durable recovery fallback.
...
- Existing Laravel webhook routes `/webhook/new` and `/webhook/edit` use the legacy worker path and must not be extended for the new architecture. The new target endpoint is `/api/webhooks/paperless`.
- Laravel webhook ingestion currently persists durable state and optionally attempts direct enqueue when `ARCHIBOT_WEBHOOK_ENQUEUE_COMMAND` is configured. Recovery scan and entrypoint remain the durable fallback from queued `webhook_deliveries` to Dramatiq actors; the next safe milestone is a live Docker/RabbitMQ/PostgreSQL smoke check.
...
- Pending document runs now reach a document actor and perform read-only Paperless fetch, LLM classification, and Laravel review suggestion persistence. Accepted suggestions now have an event-driven commit actor. Commit only uses reviewed existing IDs and preserves existing Paperless storage paths.
