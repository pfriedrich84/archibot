...
- **Model confidence must not authorize Paperless writes.** ADR-0018 requires `auto_commit_confidence` to be disabled; implementation is pending, and the current runtime can still honor configured thresholds until containment milestone 0.2 lands. Do not extend that behavior.
...
- **Runtime ownership has one durable model.** ADR-0017 removes productive SQLite processing, makes Laravel the sole Pipeline Start owner, and makes Python the owner of processing plus domain execution lifecycle. CLI, UI and MCP must use this same model.
