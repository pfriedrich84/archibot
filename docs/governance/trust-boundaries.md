...
| Absurd queue | PostgreSQL-backed event-driven execution transport for Python actors. | PostgreSQL queue connection URL, queue payload references, not full product source-of-truth state. | ADR-0013 owns the boundary. PostgreSQL remains the durable source of truth. |
...
| Docker volumes `/data` and PostgreSQL/Absurd queue tables | Persistent runtime state. | Config exports, prompt overrides, DB and queue state. | Do not delete or overwrite persistent volumes without explicit operator approval. Reset is CLI-only and destructive by design. |
