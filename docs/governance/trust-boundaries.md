...
| Laravel database queues | Event-driven execution transport for fixed Python actor commands. | PostgreSQL/Laravel database connection, queue payload references, not full product source-of-truth state. | ADR-0015 owns the target boundary. PostgreSQL pipeline tables remain the durable source of truth. |
