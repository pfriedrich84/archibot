...
The implementation state and sequencing for these accepted controls are tracked in [`../implementation-plan-security-architecture-hardening.md`](../implementation-plan-security-architecture-hardening.md). Chat/RAG containment (0.1), confidence auto-commit containment (0.2), local-only live-authorized OCR review containment (0.3), fail-closed webhook ingress (0.4), and deployment-pinned first-run setup hardening (0.6) are implemented; reviewers must continue treating the remaining milestone-0 gaps as open risks.
