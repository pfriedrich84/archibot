...
| AI providers: Ollama-compatible and OpenAI-compatible endpoints | OCR, classification, embeddings and judge. Chat/RAG calls are disabled. | May receive document text/OCR content depending on active role and provider profile. API keys may be configured for compatible providers. | Local-first is default. Cloud provider use requires explicit approval and documentation. Provider settings are audited and secrets are write-only. Issue #221 is the only Chat/RAG redesign/re-enable track. |
