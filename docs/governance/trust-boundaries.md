...
These controls are implemented and remain mandatory. Current changes are governed by the controls themselves, accepted ADRs, and the review rule above.
...
- Laravel Database Queues are the only transport, Laravel owns Pipeline Start, Python owns domain execution lifecycle, and PostgreSQL/pgvector are the only productive state/search model under ADR-0017.
